...
export function Waves({
    ...
    // Setup lines - adaptive density based on device
    const setLines = useCallback(() => {
        ...
        // Adaptive spacing: sparser on mobile for better performance
        const isMobile = width < 768
        isMobileRef.current = isMobile
        const xGap = isMobile ? 55 : 35
        const yGap = isMobile ? 60 : 40
    ...
    // Move points - reduced wave amplitude on mobile
    const movePoints = (time: number) => {
        ...
        // Reduced wave on mobile
        const ampMultiplier = isMobile ? 0.6 : 1
        ...
        for (let li = 0; li < lines.length; li++) {
            ...
            for (let pi = 0; pi < points.length; pi++) {
                ...
                p.wave.x = Math.cos(move) * 12 * ampMultiplier
                p.wave.y = Math.sin(move) * 6 * ampMultiplier
