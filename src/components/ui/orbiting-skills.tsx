...
// --- Type Definitions ---
export type IconType = 'html' | 'css' | 'javascript' | 'react' | 'node' | 'tailwind';
export type GlowColor = 'cyan' | 'purple' | 'sage';
...
const skillsConfig: SkillConfig[] = [
  ...
  { id: 'node', orbitRadius: 180, size: 45, speed: -0.6, iconType: 'node', phaseShift: (2 * Math.PI) / 3, glowColor: 'sage', label: 'Node.js' },
  { id: 'tailwind', orbitRadius: 180, size: 40, speed: -0.6, iconType: 'tailwind', phaseShift: (4 * Math.PI) / 3, glowColor: 'cyan', label: 'Tailwind CSS' },
];

// --- CSS-driven Orbiting Skill (no React state updates) ---
const OrbitingSkill = memo(({ config }: OrbitingSkillProps) => {
  ...
  useEffect(() => {
    ...
    const animate = (timestamp: number) => {
      ...
      const angle = elapsed * speed + phaseShift;
  ...
  return (
    ...
    >
      <div
        className="orbit-skill-icon relative w-full h-full p-2 bg-gray-800/90 backdrop-blur-sm rounded-full flex items-center justify-center shadow-lg cursor-pointer"
      >
        <SkillIcon type={iconType} />
        <div className="orbit-skill-label absolute -bottom-8 left-1/2 -translate-x-1/2 px-2 py-1 bg-gray-900/95 backdrop-blur-sm rounded text-xs text-white whitespace-nowrap pointer-events-none">
          {label}
        </div>
      </div>
    </div>
...
const GlowingOrbitPath = memo(({ radius, glowColor = 'cyan', animationDelay = 0 }: GlowingOrbitPathProps) => {
  const glowColors = {
    ...
    sage: { primary: 'rgba(110, 191, 139, 0.4)', secondary: 'rgba(110, 191, 139, 0.2)', border: 'rgba(110, 191, 139, 0.3)' }
  };
...
// --- Styles for hover effects and pulse (injected once) ---
const ORBIT_STYLES = `
.orbit-skill-label {
  opacity: 0;
...
.orbit-skill-icon:hover .orbit-skill-label {
  opacity: 1;
}
.orbit-pulse {
...
export default function OrbitingSkills() {
  const orbitConfigs: Array<{ radius: number; glowColor: GlowColor; delay: number }> = [
    ...
    { radius: 180, glowColor: 'sage', delay: 1.5 }
  ];
    ...
    <div className="relative w-full h-full min-h-[500px] flex items-center justify-center overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-10">
        <div 
          ...
          style={{
            backgroundImage: `radial-gradient(circle at 25% 25%, var(--color-sage) 0%, transparent 50%),
                             radial-gradient(circle at 75% 75%, var(--color-mist) 0%, transparent 50%)`,
          }}
        />
      </div>
      ...
      <div className="relative w-[300px] h-[300px] md:w-[450px] md:h-[450px] flex items-center justify-center">
        {/* Central "Code" Icon */}
        <div className="w-20 h-20 bg-gradient-to-br from-surface to-background rounded-full flex items-center justify-center z-10 relative shadow-2xl border border-border">
          <div className="absolute inset-0 rounded-full bg-sage/30 blur-xl orbit-pulse"></div>
          <div className="absolute inset-0 rounded-full bg-mist/20 blur-2xl orbit-pulse" style={{ animationDelay: '1s' }}></div>
        ...
        {/* Orbiting skill icons — animated via rAF + direct DOM, no React state */}
