...
class AnimationController {
  ...
  constructor(canvas: HTMLCanvasElement, ctx: CanvasRenderingContext2D, dpr: number, size: number, isMobile: boolean) {
    this.canvas = canvas; this.ctx = ctx; this.dpr = dpr; this.size = size
    // Reduce star count on mobile for smoother performance
    this.numberOfStars = isMobile ? 1500 : 4000
  ...
  private setupTimeline() {
    this.timeline.to(this, { time: 1, duration: 15, ease: "none" })
  }
...
export function SpiralAnimation() {
  ...
  useEffect(() => {
    // Debounced resize handler to avoid excessive recreations
    const handleResize = () => {
  ...
  useEffect(() => {
    ...
    const renderLoop = () => {
      if (animationRef.current) animationRef.current.render()
      animationFrameId = requestAnimationFrame(renderLoop)
    }

    const dpr = Math.min(window.devicePixelRatio || 1, 2) // Cap DPR at 2 for performance
    // Cap canvas size to prevent massive buffers on large screens
    const maxCanvasDim = isMobile ? 1200 : 2400
    const size = Math.min(Math.max(dimensions.width, dimensions.height) * 1.2, maxCanvasDim)
  ...
  return (
    <div ref={containerRef} className="relative w-full h-full flex items-center justify-center pointer-events-none absolute inset-0 overflow-hidden">
      <canvas ref={canvasRef} className="absolute z-0 mix-blend-screen will-change-transform" />
    </div>
  )
}
